...
document.querySelector('#submitbtn').addEventListener('click',function(e){
    ...
    if(name && date && amount){
        table.innerHTML += '<tr><td>'+MonthArray[month-1]+'</td><td>'+category+'</td><td>'+name+'</td><td>$'+amount+'</td><td>'+ outputDate+'</td><td><button class="remove" onclick="this.parentNode.parentNode.remove();">X</button></td></tr>';
        ...
        localStorage.setItem("expenses",JSON.stringify(dataBase));
});
...
function getJson()
{
    const jsonData= JSON.stringify( JSON.parse(localStorage.getItem("expenses")),null,2);
...
{
    document.getElementById('jsonText').innerHTML = localStorage.getItem("expenses");
})